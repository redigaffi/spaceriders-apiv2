...
from core.shared.ports import ChainServicePort, PlanetRepositoryPort, ResponsePort
...
class NftData:
    ...
    async def planet_nft_view(self, planet_id: str):
        ...
        attributes = [
            OpenseaAttributeStandardResponse(
                display_type="string",
                trait_type="Position",
                value=f"{planet.position}:{planet.solar_system}:{planet.galaxy}",
            ),
            OpenseaAttributeStandardResponse(
                display_type="number", trait_type="Level", value=str(planet.level)
            ),
            OpenseaAttributeStandardResponse(
                display_type="number",
                trait_type="Experience",
                value=str(planet.experience),
            ),
            OpenseaAttributeStandardResponse(
                display_type="number", trait_type="Diameter", value="planet.diameter"
            ),
            OpenseaAttributeStandardResponse(
                display_type="number", trait_type="Slots", value=str(planet.slots)
            ),
            OpenseaAttributeStandardResponse(
                display_type="number",
                trait_type="Slots Used",
                value=str(planet.slots_used),
            ),
            OpenseaAttributeStandardResponse(
                display_type="number",
                trait_type="Minimum Temperature",
                value=str(planet.min_temperature),
            ),
            OpenseaAttributeStandardResponse(
                display_type="number",
                trait_type="Maximum Temperature",
                value=str(planet.max_temperature),
            ),
            OpenseaAttributeStandardResponse(
                display_type="number",
                trait_type="Metal Reserve",
                value=str(planet.reserves.total_metal),
            ),
            OpenseaAttributeStandardResponse(
                display_type="number",
                trait_type="Petrol Reserve",
                value=str(planet.reserves.total_petrol),
            ),
            OpenseaAttributeStandardResponse(
                display_type="number",
                trait_type="Crystal Reserve",
                value=str(planet.reserves.total_crystal),
            ),
        ]

        image_name = f"{planet.image}-{planet.rarity}-bg.webp"
        response = OpenseaMetadataNftResponse(
            description="Deep space is dangerous, this is your home, fight for it.",
    ...
    async def testnet_ticket_nft(self, token_id: int):
        ticket_info = await self.chain_service_port.spaceriders_ticket_nft_call(
            "byTokenIdIdData", token_id
        )

        royalty_fee_bips = await self.chain_service_port.spaceriders_ticket_nft_call(
            "royaltyFeeBips"
        )
        royalty_receiver = await self.chain_service_port.spaceriders_ticket_nft_call(
            "royaltyReceiver"
        )
        ...
        attributes = [
            OpenseaAttributeStandardResponse(
                display_type="number", trait_type="Generation", value=str(generation)
            ),
            OpenseaAttributeStandardResponse(
                display_type="", trait_type="Lifetime Access", value=life_time_str
            ),
        ]
        ...
        anim_url = (
            f"{self.testnet_ticket_images_base_url}/{tmp_lifet}_gen_{generation}.mp4"
        )
        img_url = (
            f"{self.testnet_ticket_images_base_url}/{tmp_lifet}_gen_{generation}.jpg"
        )
