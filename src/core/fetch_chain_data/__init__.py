...
from core.shared.ports import ChainServicePort, ResponsePort, TokenPricePort
...
class FetchChainData:
    ...
    async def get_chain_data(self):
        ...
        pair_address = await self.contract_service.get_contract_address(
            ChainServicePort.PAIR_CONTRACT
        )
        router_address = await self.contract_service.get_contract_address(
            ChainServicePort.ROUTER_CONTRACT
        )
        token_contract = await self.contract_service.get_contract_address(
            ChainServicePort.SPACERIDERS_TOKEN_CONTRACT
        )
        game_contract = await self.contract_service.get_contract_address(
            ChainServicePort.SPACERIDERS_GAME_CONTRACT
        )
        nft_contract = await self.contract_service.get_contract_address(
    ...
    async def get_chain_token_price(self):
        ...
        return await self.response_port.publish_response(
            FetchChainTokenPriceResponse(spr_price_usd=token_price)
        )
