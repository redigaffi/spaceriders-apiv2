...
from core.shared.models import AppBaseException, ShadyActivityException, BKMTransaction
...
class PlanetBKM:
    ...
    async def recover_transactions(self, request: RecoverBKMTransactionRequest) -> list[BKMTransaction]:

        planet = await self.planet_repository_port.get(request.planet_id)

        energy_deposit_count = await self.contract_service.spaceriders_game_call("getBkmTransactionMapCount", str(request.planet_id))

        result = []
        for i in range(energy_deposit_count):
            energy_deposit_id_sm = await self.contract_service.spaceriders_game_call("bkmTransactionMap", str(request.planet_id), i)
            energy_deposit = await self.bkm_repository_port.get(energy_deposit_id_sm)

            if energy_deposit is None:
                await self.logging_port.info("Found a $BKM transaction on blockchain which was not in our database")
                energy_deposit_info = await self.contract_service.spaceriders_game_call("bkmTransactions", energy_deposit_id_sm)
                ...
                energy_deposit = BKMTransaction(request_id=energy_deposit_id_sm,
                ...
                energy_deposit = await self.bkm_repository_port.create_bkm_transaction(energy_deposit)
    ...
    async def create_transaction(self, user: str, request: BKMTransactionRequest) -> BKMTransaction:
        ...
        bkm_deposit_info = await self.contract_service.spaceriders_game_call("bkmTransactions", request.deposit_id)
        ...
        bkm_deposit = BKMTransaction(request_id=request.deposit_id,
                                     created_time=now,
                                     token_amount=token_amount,
                                     planet_id=request.planet_id,
                                     was_recovered=False,
